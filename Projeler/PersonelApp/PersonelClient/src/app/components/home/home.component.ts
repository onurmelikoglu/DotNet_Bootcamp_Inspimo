...
export class HomeComponent {
    ...
    @ViewChild("certificatedFile") certificatedFile: ElementRef<HTMLInputElement> |undefined;
    @ViewChild("diplomaFile") diplomaFile: ElementRef<HTMLInputElement> |undefined;
    @ViewChild("healtReportFile") healtReportFile: ElementRef<HTMLInputElement> |undefined;


    constructor(private date: DatePipe){
      this.addModel.avatar = "/assets/upload.jpeg";
      this.addModel.startDate = this.date.transform(new Date(), "yyyy-MM-dd");
      this.addModel.certificatedFile = [];
    }
    ...
    openCertificatedFile(){
      this.certificatedFile?.nativeElement.click();
    }

    setCertificatedFile(event:any){
      this.addModel.certificatedFile.push(event.target.files[0]);
      console.log("sertifikalar",this.addModel.certificatedFile);
    }

    openDiplomaFile(){
      this.diplomaFile?.nativeElement.click();
    }

    setDiplomaFile(event:any){
      this.addModel.diplomaFile = event.target.files[0];
      console.log("diploma",this.addModel.diplomaFile);
    }

    openHealtReportFile(){
      this.healtReportFile?.nativeElement.click();
    }

    setHealtReportFile(event:any){
      this.addModel.healtReportFile = event.target.files[0];
      console.log("sağlık raporu",this.addModel.healtReportFile);
    }
